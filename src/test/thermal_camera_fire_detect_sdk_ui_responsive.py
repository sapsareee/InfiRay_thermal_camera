...
class MetricCard(QFrame):
    def __init__(self, title, value="--", unit="", large=False, accent="normal"):
        ...
        self.accent = accent
        self.large = large
        self.value_font_size = 30 if self.large else 17
        self.setup_ui()

    def setup_ui(self):
        self.setObjectName("metricCard")
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.setMinimumHeight(72 if self.large else 58)
        layout = QVBoxLayout(self)
        layout.setContentsMargins(10, 7, 10, 7)
        layout.setSpacing(2)

        self.title.setAlignment(Qt.AlignLeft)
        self.title.setStyleSheet("color: #9fb3c8; font-size: 9pt; font-weight: 600;")
        ...
        self.value.setStyleSheet(
            f"color: #eef6ff; font-size: {self.value_font_size}pt; "
            "font-weight: 800; font-family: Consolas;"
        )
        self.unit.setAlignment(Qt.AlignBottom)
        self.unit.setStyleSheet("color: #b6c5d6; font-size: 10pt; font-weight: 600;")
    ...
    def set_alert_level(self, level):
        if level == "critical":
            self.value.setStyleSheet(
                f"color: #ff3b30; font-size: {self.value_font_size}pt; "
                "font-weight: 900; font-family: Consolas;"
            )
        elif level == "warning":
            self.value.setStyleSheet(
                f"color: #ffcc00; font-size: {self.value_font_size}pt; "
                "font-weight: 900; font-family: Consolas;"
            )
        elif level == "safe":
            self.value.setStyleSheet(
                f"color: #30d158; font-size: {self.value_font_size}pt; "
                "font-weight: 900; font-family: Consolas;"
            )
        else:
            self.value.setStyleSheet(
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.status_label.setStyleSheet(
            "background-color: #1e293b; color: #e5edf7; border-radius: 12px; "
        ...
        self.video_label.setMinimumSize(*MIN_VIDEO_SIZE)
        ...
        for index, card in enumerate([
            ...
            area_grid.addWidget(card, index // 3, index % 3)
        ...
        for index, card in enumerate([
            ...
            env_grid.addWidget(card, 0, index)
        ...
        self.mission_label.setStyleSheet(
            "background-color: #1e293b; border-radius: 10px; padding: 10px; "
            "font-size: 18pt; font-weight: 900; color: #dbeafe;"
        )
    ...
    def resize_to_screen(self):
        ...
        if screen is None:
            self.setGeometry(60, 40, 1280, 720)
            return

        available = screen.availableGeometry()
        width = min(1280, int(available.width() * 0.95))
        height = min(720, int(available.height() * 0.92))
    ...
    def render_latest_image(self):
        ...
        pixmap = self.convert_cv_to_qt(img_to_show)
    ...
    def update_fire_status(self, is_fire):
        ...
        if is_fire:
            ...
            self.mission_label.setStyleSheet(
                "background-color: #7f1d1d; border-radius: 10px; padding: 10px; "
                "font-size: 18pt; font-weight: 900; color: #fde68a;"
            )
        else:
    ...
    def update_fire_banner(self):
        if self.current_fire:
            ...
            self.status_label.setStyleSheet(
                f"background-color: {background}; color: #fff7ed; border-radius: 12px; "
                "font-size: 23pt; font-weight: 950; padding: 8px;"
            )
        else:
